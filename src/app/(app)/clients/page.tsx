...
function NewClientForm() {
  return <ToastForm action={createClientAction} successMessage="Client added" className="grid">
    ...
    <button className="btn btn-primary" style={{ justifySelf: "end" }}>Add client</button>
  </ToastForm>;
}

export default async function ClientsPage() {
  const rows = await db.select().from(clients).where(eq(clients.archived, false)).orderBy(clients.name);
  return <><PageHeader eyebrow="Relationships" title="Clients" action={<AddModal title="Add a client" triggerLabel="Add client"><NewClientForm/></AddModal>}/><div className="content">
