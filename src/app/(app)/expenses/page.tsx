...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<{ view?: string; error?: string }> }) {
  ...
  const newExpenseForm=<ToastForm action={createExpenseAction} successMessage="Expense added" className="grid">
      ...
      <section>
        ...
        <div className="card">{rows.length ? rows.map(({ expense, client, project, invoiceId, invoiceNumber }) => {
          ...
          return <div key={expense.id} style={{ padding: 18, borderBottom: "1px solid var(--line)" }}>
            ...
            {!billed && <details style={{ marginTop: 9 }}><summary className="small" style={{ cursor: "pointer", color: "var(--accent)", fontWeight: 700 }}>Edit expense</summary><ToastForm action={updateExpenseAction} successMessage="Expense saved" className="grid" style={{ paddingTop: 12 }}><input type="hidden" name="id" value={expense.id}/>
            ...
            </ToastForm><form action={deleteExpenseAction} style={{ marginTop: 8 }}><input type="hidden" name="id" value={expense.id}/><button className="btn btn-danger"><Trash2 size={14}/> Delete expense</button></form></details>}
          </div>;
